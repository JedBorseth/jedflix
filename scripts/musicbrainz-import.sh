...
"${COMPOSE[@]}" up -d musicbrainz-db

echo "== Waiting for Postgres =="
for i in $(seq 1 60); do
  if "${COMPOSE[@]}" exec -T musicbrainz-db pg_isready -U musicbrainz -d musicbrainz >/dev/null 2>&1; then
    break
  fi
  sleep 2
done
"${COMPOSE[@]}" exec -T musicbrainz-db pg_isready -U musicbrainz -d musicbrainz
