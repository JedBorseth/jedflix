import { AppLink } from "@/components/layout/AppLink";
import { ProgressiveCoverImage } from "@/components/browse/ProgressiveCoverImage";
import { Button } from "@/components/ui/button";
import type { AlbumItem } from "@/lib/spotify";
import { getAlbumDetailPath, getArtistPath } from "@/lib/spotify";

type MusicHeroBannerProps = {
  album: AlbumItem;
};

export function MusicHeroBanner({ album }: MusicHeroBannerProps) {
  const artists = album.artists.length > 0 ? album.artists.join(", ") : null;
  const primaryArtistId = album.artistIds[0];

  return (
    <section className="relative h-[70vh] min-h-[420px] w-full overflow-hidden">
      <ProgressiveCoverImage
        src={album.imageUrl}
        alt=""
        className="absolute inset-0 h-full w-full scale-110 object-cover blur-xl"
      />
      <div className="absolute inset-0 bg-gradient-to-r from-black via-black/80 to-black/40" />
      <div className="absolute inset-0 bg-gradient-to-t from-zinc-950 via-transparent to-black/40" />

      <div className="pt-navbar relative z-10 flex h-full max-w-5xl items-end gap-8 px-4 pb-16 md:px-12">
        <ProgressiveCoverImage
          src={album.imageUrl}
          alt={album.name}
          className="hidden aspect-square h-auto w-44 shrink-0 self-end rounded-md object-cover shadow-2xl md:block"
        />
        <div className="flex max-w-2xl flex-col justify-end">
          <p className="mb-2 text-sm font-medium uppercase tracking-widest text-red-500">
            Featured
          </p>
          <h1 className="mb-4 text-4xl font-bold text-white md:text-6xl">{album.name}</h1>
          {artists ? (
            <p className="mb-3 text-sm text-zinc-300 md:text-base">{artists}</p>
          ) : null}
          <p className="mb-4 line-clamp-3 text-sm text-zinc-200 md:text-base">
            Discover this album and more music from Spotify.
          </p>
          <div className="mb-6 flex flex-wrap gap-3 text-sm text-zinc-300">
            {album.year ? <span>{album.year}</span> : null}
            {album.albumType ? <span className="capitalize">{album.albumType}</span> : null}
            {album.totalTracks ? <span>{album.totalTracks} tracks</span> : null}
            {album.genres[0] ? <span className="capitalize">{album.genres[0]}</span> : null}
          </div>
          <div className="flex flex-wrap items-start gap-3">
            <Button asChild size="lg" className="bg-white text-black hover:bg-zinc-200">
              <AppLink to={getAlbumDetailPath(album)} state={{ preview: album }}>
                More Info
              </AppLink>
            </Button>
            {primaryArtistId ? (
              <Button asChild size="lg" variant="outline" className="border-zinc-600">
                <AppLink
                  to={getArtistPath(primaryArtistId)}
                  state={{
                    preview: {
                      id: primaryArtistId,
                      name: album.artists[0] ?? "Artist",
                      imageUrl: album.imageUrl,
                      genres: album.genres ?? [],
                    },
                  }}
                >
                  View Artist
                </AppLink>
              </Button>
            ) : null}
          </div>
        </div>
      </div>
    </section>
  );
}
