...
- Go backend discovers magnets via Torrentio, resolves Real Debrid streams server-side for the web client, proxies Open Library with a 12-hour in-memory cache, serves music catalog metadata via MusicBrainz (+ Cover Art Archive; Last.fm enriches shelves/images/top tracks), resolves music audio via yt-dlp (prefer m4a/mp3/aac over webm/opus for browser/Safari), and applies configurable size/seeders limits.
...
- Movie/TV metadata, cast, and search use TMDB; audiobook metadata uses Open Library (subjects plus trending/weekly for browse rows); music catalog uses MusicBrainz via the Go backend (MBIDs for artists/albums/recordings; Cover Art Archive for album art; Last.fm optional for charts/images/recommendations). Spotify OAuth remains for party mode and playlist sync only.
